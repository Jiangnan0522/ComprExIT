...
# Logging backend for HuggingFace Trainer
# Options: wandb, tensorboard, none
report_to="wandb"

##### wandb run name ####
run_name="512-128-icae-gpu"

# Multi-node setup
model_structure="icae"
nnodes=1
nproc_per_node=2
...
model_name_or_path="${1:?Usage: pass <base_model> as the first arg (local path or HF repo id, e.g. meta-llama/Llama-3.2-1B)}"
...
num_hidden_layers=0 # @@CRITICAL ARG@@
top_k_layers=-1 # @@CRITICAL ARG@@

# ICAE 默认训练方式：冻结 LLM、训练 compressor(+LoRA) 与 memory embeddings
training_freezing_mode=llm  # Options: compress+llm, both, compress, llm, projector, none
