...
def get_model_and_tokenizer(model_args, device="cpu", evaluation_mode=True, return_tokenizer_only=False, **kwargs):    
    ...
    if model_args_dict.get("fix_mistral_regex"):
        # Fix for certain Mistral-family tokenizers with incorrect regex in tokenizer.json.
        # Some Transformers versions don't support this kwarg; we handle that below.
    ...
    try:
        ...
        # Backward-compat: older Transformers may not recognize fix_mistral_regex.
        if "fix_mistral_regex" in tokenizer_kwargs:
            logger.warning(
                f"AutoTokenizer.from_pretrained does not support fix_mistral_regex in this Transformers version. "
