...
class LlamaConfig(PretrainedConfig):
    ...
    def _rope_scaling_validation(self):
        ...
        # Support both old format (type, factor) and new Llama3 format (rope_type, factor, etc.)
        rope_scaling_type = self.rope_scaling.get("type", self.rope_scaling.get("rope_type", None))
        rope_scaling_factor = self.rope_scaling.get("factor", None)
        
        # For Llama3 RoPE scaling format, just check that factor exists
        if "rope_type" in self.rope_scaling:
            if rope_scaling_factor is None or not isinstance(rope_scaling_factor, float) or rope_scaling_factor <= 1.0:
                raise ValueError(f"`rope_scaling`'s factor field must be a float > 1, got {rope_scaling_factor}")
            return
        
        # For old format, validate strictly
