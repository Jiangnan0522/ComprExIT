...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def generate(self, *args, reset_beacon_memory: bool = True, **kwargs):
        """
        Wrapper around HF `GenerationMixin.generate` that can optionally reset beacon memory.

        NOTE: We intentionally call `GenerationMixin.generate(self, ...)` directly instead of
        `super().generate(...)`. In notebook workflows with `%autoreload`, class objects can be
        redefined while existing model instances still point to the old class, and zero-arg
        `super()` can raise `TypeError: super(type, obj): obj must be an instance or subtype of type`.
        Calling the mixin implementation directly is robust to that situation.
        """
        if reset_beacon_memory and hasattr(self, "memory"):
            self.memory.reset()
