...
class Memory(torch.nn.Module):
    ...
    def prepare(self, input_ids, attention_mask, labels, skip_first=None, skip_last=None):
        ...
        # IMPORTANT:
        # `config.vocab_size` can change at runtime (e.g. SFT adds a new <pad> token then calls
        # `model.resize_token_embeddings(len(tokenizer))`). `Memory.beacon_token` must track the *current*
        # `config.vocab_size`, otherwise the old beacon id can become equal to the new pad id (vocab_size - 1),
        # causing pads to be mistaken as beacons and leading to out-of-range embedding indices.
        self.beacon_token = torch.tensor([self.config.vocab_size], device=self._cpu)
        if getattr(self.config, "pad_token_id", None) is not None:
            assert self.beacon_token.item() != self.config.pad_token_id, (
                f"Invalid configuration: beacon_token_id ({self.beacon_token.item()}) equals pad_token_id "
                f"({self.config.pad_token_id}). This usually happens if vocab_size changed but beacon_token_id "
                f"was not refreshed."
            )
