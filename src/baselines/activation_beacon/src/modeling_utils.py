...
def evaluate_perplexity(model, dataloader, accelerator:Optional[Accelerator]=None):
    ...
    for i, x in enumerate(tqdm(dataloader, desc="Computing Perplexity")):
        ...
        if index is None:
            ...
            # index = torch.arange(start_index, start_index + batch_size, device=x["input_ids"].device) + process_index * 10000000
        ...
        if getattr(output, "batch_loss", None) is not None:
...
def get_rope(head_dim, base, max_position_embeddings, rope_scaling=None):
    ...
    if rope_scaling is None:
        ...
        # Support both old format (type) and new Llama3 format (rope_type)
