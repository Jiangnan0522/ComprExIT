...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        if beacon_size > 0:
            # See `llama/modeling_llama.py` for rationale.
            ...
            beacon_embed_indices = (input_ids - self.config.vocab_size).clamp(min=0, max=self.beacon_embed_tokens.num_embeddings - 1)
            beacon_input_embeds = self.beacon_embed_tokens(beacon_embed_indices)

            inputs_embeds = torch.where(is_beacon_token.unsqueeze(-1), beacon_input_embeds, ordinal_inputs_embeds)
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        # Ensure generation_config is initialized for GenerationMixin
