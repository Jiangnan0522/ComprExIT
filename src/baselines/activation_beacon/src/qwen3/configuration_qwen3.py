...
# layer_type_validation and RopeParameters were added in newer transformers versions.
# Provide safe fallbacks so this config works with older installs too.
try:
    ...
    def layer_type_validation(layer_types, num_hidden_layers):
        ...
        for lt in layer_types:
            if lt not in valid:
                raise ValueError(f"Unknown layer type '{lt}'. Expected one of {valid}.")

try:
    from transformers.modeling_rope_utils import RopeParameters
except ImportError:
    # Use a plain dict as the type when RopeParameters isn't available.
    RopeParameters = dict
...
class Qwen3Config(PreTrainedConfig):
    ...
    def __init__(
        ...
        rope_parameters: Optional[Union[RopeParameters, Dict[str, RopeParameters]]] = None,
        ...
        **kwargs,
        ...
        self.beacon_ratio = beacon_ratio if beacon_ratio is not None else [2, 4, 8, 16, 32]
        self.beacon_ratio_mix = beacon_ratio_mix
        self.beacon_param = beacon_param if beacon_param is not None else []
