...
class ActivationBeaconTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if "labels" not in inputs or inputs["labels"][0] is None:
        ...
        # Call parent's compute_loss with appropriate arguments based on transformers version
        if num_items_in_batch is not None:
            outputs = super().compute_loss(model, inputs, return_outputs, num_items_in_batch)
        else:
    ...
    def _get_train_sampler(self, train_dataset: Dataset = None) -> Optional[torch.utils.data.Sampler]:
        ...
        if self.args.group_by_stride is not None:
            if is_datasets_available() and isinstance(self.train_dataset, datasets.Dataset):
                if (self.args.length_column_name is not None) and (self.args.length_column_name in train_dataset.column_names):
    ...
    @torch.no_grad()
    def evaluate(self, eval_dataset: Dataset = None, ignore_keys: List[str] = None, metric_key_prefix: str = "eval") -> Dict[str, float]:        
        ...
        self.model.memory.reset()
        train_beacon_ratio = self.model.memory.config.beacon_ratio
        train_beacon_ratio_mix = self.model.memory.config.beacon_ratio_mix
